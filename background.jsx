...
function Loader({ gone }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black flex flex-col items-center justify-center gap-7 transition-opacity duration-700"
      ...
      <div className="font-mono text-[11px] text-zinc-500 tracking-[0.3em]">COSMO BLOOM · USTANAWIANIE ORBITY</div>
    </div>
  );
}
...
function FAQItem({ question, answer }) {
  ...
  return (
    <div className="border-t border-white/10 py-[22px] cursor-pointer group" onClick={() => setOpen(!open)}>
      ...
      <div className="text-zinc-400 leading-relaxed text-[0.95rem] overflow-hidden transition-all duration-400"
  ...
  return (
    <div className="bento-card group relative overflow-hidden">
      ...
      <div className="flex items-baseline justify-between mb-2">
        ...
        <span className="font-mono text-[10px] tracking-[0.2em] text-zinc-500 uppercase">{sector}</span>
      </div>
      <div className="flex items-end gap-3 mt-4 pt-4 border-t border-white/10">
        ...
        <span className="font-mono text-[10px] tracking-[0.2em] text-zinc-400 uppercase pb-2">{metricLabel}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative font-sans text-white bg-black" style={{ '--accent-hue': tweaks.accentHue }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-4 py-4 md:px-12 md:py-6 pointer-events-none">
        ...
        <nav className="hidden xl:flex items-center gap-9 font-mono text-[11px] tracking-[0.2em] pointer-events-auto px-6 py-2.5 rounded-full bg-black/60 border border-white/10 backdrop-blur-xl">
          {navItems.map(item => (
      ...
      <main className="relative z-10">
        ...
        <section id="hero" className="min-h-screen flex items-center px-[6vw] pt-[12vh]">
          <div className="max-w-4xl relative">
            ...
            {/* Hero stat strip */}
            <div className="mt-20 flex flex-wrap gap-x-12 gap-y-5 font-mono text-[10px] tracking-[0.25em] text-zinc-500 uppercase max-w-[700px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-10">
              {[
                ...
                <div key={v.num} className="border border-white/10 rounded-2xl p-5 bg-white/5 hover:border-pink-1 transition-all">
                  ...
                  <p className="text-[0.8rem] text-zinc-400 mt-2 leading-relaxed">{v.text}</p>
                </div>
              ))}
            </div>
          </Reveal>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-16">
            <Reveal>
              <div className="lg:sticky lg:top-32">
                ...
                <div className="mt-8 font-mono text-[10px] tracking-[0.25em] text-zinc-500 space-y-2">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                { step: '02', title: 'Prototypowanie', desc: 'Szybkie iteracje designu. Zobacz i poczuj swój produkt w 7 dni.', days: 'TYDZIEŃ 2–3' },
                { step: '03', title: 'Engineering', desc: 'Ręcznie pisany kod, czysta architektura i najwyższa wydajność.', days: 'TYDZIEŃ 4–7' },
                { step: '04', title: 'Wdrożenie & Skalowanie', desc: 'Monitoring wyników i ciągła optymalizacja dla maksymalnych zysków.', days: 'TYDZIEŃ 8+' },
              ].map((p, i) => (
                <Reveal key={i} delay={i * 80}>
                  <div className="bento-card flex flex-col items-start gap-5 group h-full">
                    <div className="w-full flex items-center justify-between">
                      ...
                      <div className="font-mono text-[9px] tracking-[0.25em] text-zinc-500">{p.days}</div>
                    </div>
                    <h3 className="text-xl font-bold text-white">{p.title}</h3>
                    <p className="text-sm text-zinc-300 leading-relaxed group-hover:text-white transition-colors">{p.desc}</p>
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        <section id="opinie" className="min-h-screen px-[6vw] py-[12vh]">
          <div className="max-w-[1200px] mx-auto w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[18px]">
              {[
                { name: 'Marta Kuznetsova', role: 'CEO · Northwind Labs', quote: 'Cosmo Bloom to nie tylko studio, to partnerzy, którzy przenieśli nasz design na wyższy poziom. Konwersja wzrosła o 240%.', initials: 'MK' },
                { name: 'Piotr Janczak', role: 'CTO · Aurelia Capital', quote: 'Dbałość o detale i jakość kodu na poziomie, którego nie spotkałem wcześniej u żadnej agencji zewnętrznej.', initials: 'PJ' },
              ].map((q, i) => (
                <Reveal key={i}>
        ...
        <section id="kontakt" className="min-h-screen px-[6vw] py-[12vh] flex items-center">
          <div className="max-w-[1200px] mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <Reveal>
              ...
              <p className="text-zinc-400 text-lg leading-relaxed mb-12 max-w-[44ch]">
              ...
              <div className="mt-12 p-5 rounded-2xl border border-white/10 bg-white/[0.02] font-mono text-[10px] tracking-[0.25em] text-zinc-500 max-w-[420px]">
            ...
            <Reveal className="bg-white/5 border border-white/10 p-10 md:p-12 rounded-[40px] backdrop-blur-2xl">
              <form onSubmit={handleSubmit} className="space-y-7">
                <div className="flex flex-col gap-2">
                  ...
                  <input required className="bg-transparent border-b border-white/20 py-3 outline-none focus:border-pink-1 transition-colors text-white placeholder:text-zinc-600" placeholder="Jan Kowalski" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-[10px] text-zinc-400 uppercase tracking-[0.3em]">Email</label>
                  <input required type="email" className="bg-transparent border-b border-white/20 py-3 outline-none focus:border-pink-1 transition-colors text-white placeholder:text-zinc-600" placeholder="ty@firma.pl" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-[10px] text-zinc-400 uppercase tracking-[0.3em]">Budżet (orientacyjnie)</label>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea required className="bg-transparent border-b border-white/20 py-3 outline-none focus:border-pink-1 transition-colors min-h-[100px] text-white placeholder:text-zinc-600" placeholder="Strona webowa, termin marzec…" />
                </div>
      ...
      <footer className="px-[6vw] py-16 border-t border-white/5 relative z-10">
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <nav className="flex gap-8 font-mono text-[10px] tracking-widest text-zinc-500">
            <a href="#" className="hover:text-white transition-colors">LINKEDIN</a>
            <a href="#" className="hover:text-white transition-colors">DRIBBBLE</a>
            <a href="#" className="hover:text-white transition-colors">INSTAGRAM</a>
            <a href="#" className="hover:text-white transition-colors">GITHUB</a>
          </nav>
          <div className="font-mono text-[9px] text-zinc-600 uppercase tracking-[0.4em]">ZBUDOWANE W KOSMOSIE · WARSZAWA</div>
        </div>
        <div className="max-w-[1200px] mx-auto mt-10 pt-6 border-t border-white/5 flex justify-between font-mono text-[9px] tracking-[0.3em] text-zinc-700 uppercase">
